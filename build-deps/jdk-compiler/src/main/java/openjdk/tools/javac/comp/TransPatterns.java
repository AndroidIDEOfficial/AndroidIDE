...
import openjdk.tools.javac.code.Symbol.DynamicVarSymbol;
...
import openjdk.tools.javac.tree.JCTree.JCParenthesizedPattern;
    ...
    private LoadableConstant toLoadableConstant(JCCaseLabel l, Type selector) {
        if (l.isPattern()) {
            ...
            if ((l.type.tsym.flags_field & Flags.ENUM) != 0) {
                ...
                LoadableConstant loadableConstant= null;
                switch (l.type.getTag()) {
                    case BYTE: CHAR:
                            SHORT: INT: 
                            loadableConstant = LoadableConstant.Int((Integer) l.type.constValue());
                            break;
                    case CLASS:
                        loadableConstant = LoadableConstant.String((String) l.type.constValue());
                        break;
                    default: throw new AssertionError();
                };
                return loadableConstant;
            }
        } else {
            return null;
        }
