...
fun Content.previousNonBlankLine(line: Int) : Int {
  for (i in line - 1 downTo 0) {
    if (getLine(i).trim { it.isWhitespace() || it == '\r' }.isNotEmpty()) {
      return i
    }
  }
 ...
 */
fun Content.getFirstNodeAtLine(tree: TSTree, line: Int) : TSNode? {
  return getFirstNodeAtLine(tree.rootNode, line)
}

/**
 * Returns the last [TSNode] at the given line number.
 */
fun Content.getLastNodeAtLine(tree: TSTree, line: Int) : TSNode? {
  return getLastNodeAtLine(tree.rootNode, line)
}

/**
 * Returns the first [TSNode] at the given line number. The leading indentation is ignored.
 */
fun Content.getFirstNodeAtLine(node: TSNode, line: Int) : TSNode? {
  ...
  return node.getNodeAt(line, (spaces + tabs) shl 1)
}
 ...
 */
fun Content.getLastNodeAtLine(node: TSNode, line: Int) : TSNode? {
  if (line < 0 || line >= lineCount) {
    return null
  }
  ...
  // we need the byte offset in the line, so we need to multiply the char offset by 2 (shl 1)
  return node.getNodeAt(line, (contentLine.length - 1) shl 1)
}