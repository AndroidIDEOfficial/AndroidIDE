...
public final class StringWrapper {
    ...
    /**
     * Reflows string literals in the given Java source code that extend past the given column
     * limit.
     */
    ...
    private static TreeRangeMap<Integer, String> getReflowReplacements(
        ...
        for (TreePath path : toFix) {
            ...
            replacements.put(
                    Range.closedOpen(
                            getStartPosition(flat.get(0)), getEndPosition(unit, getLast(flat))),
    ...
    private static ImmutableList<String> stringComponents(
        ...
        for (Tree tree : flat) {
            ...
            String text =
                    input.substring(getStartPosition(tree) + 1, getEndPosition(unit, tree) - 1);
            int start = 0;
    ...
    /**
     * Flattens the given binary expression tree, and extracts the subset that contains the given
     * path and any adjacent nodes that are also string literals.
     */
    ...
    private static String applyReplacements(
            String javaInput, TreeRangeMap<Integer, String> replacementMap)
            throws FormatterException {
