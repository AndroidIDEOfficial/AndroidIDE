...
public class ImportOrderer {
    ...
    /**
     * Reorder the inputs in {@code text}, a complete Java program. On success, another complete
     ...
     * another complete Java program is returned, which is the same as the original except the
    ...
    /**
     * {@link TokenKind}s that indicate the start of a type definition. We use this to avoid
     * scanning the whole file, since we know that imports must precede any type definition.
     */
     ...
     * https://source.android.com/setup/contribute/code-style#order-import-statements and
     * implemented in IntelliJ at
    ...
    class Import {
         ...
         * disallowed by the style guide), the trailing whitespace of the first import does not
    ...
    private ImportsAndIndex scanImports(int i) throws FormatterException {
        ...
        while (i < toks.size() && tokenAt(i).equals("import")) {
            ...
            // Gather (if any) all single line comments and accompanied line terminators following
            // this
            // import
    ...
    private String reorderedImportsString(ImmutableSortedSet<Import> imports) {
        ...
        // Pretend that the first import was preceded by another import of the same kind, so we
        // don't
     ...
     * supported: if the input is {@code import java.util.*;} then the returned string will be
     * {@code java.util.*}.
     *
