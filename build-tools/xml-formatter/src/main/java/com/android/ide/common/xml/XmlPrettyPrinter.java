...
public class XmlPrettyPrinter {
   ...
   * @deprecated Use {@link #prettyPrint(org.w3c.dom.Node, XmlFormatPreferences, XmlFormatStyle,
  ...
  /**
   * TODO: Explain why we need to do per-tag decisions on whether to keep them on the same line or
   * not. Show that we can't just do it by depth, or by file type. (style versus plurals example)
   *
  ...
  private boolean keepElementAsSingleLine(int depth, Element element) {
    ...
    return isSingleLineTag(element)
        || (mStyle == XmlFormatStyle.RESOURCE && !XmlUtils.hasElementChildren(element));
  }
