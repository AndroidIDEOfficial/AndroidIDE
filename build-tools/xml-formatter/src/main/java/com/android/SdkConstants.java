 ...
 *
 * <ul>
 *   <li><code>OS_</code> OS path constant. These paths are different depending on the platform.
 *   <li><code>FN_</code> File name constant.
 *   <li><code>FD_</code> Folder name constant.
 *   <li><code>TAG_</code> XML element tag name
 *   <li><code>ATTR_</code> XML attribute name
 *   <li><code>VALUE_</code> XML attribute value
 *   <li><code>CLASS_</code> Class name
 *   <li><code>DOT_</code> File name extension, including the dot
 *   <li><code>EXT_</code> File name extension, without the dot
 * </ul>
 */
...
public final class SdkConstants {
   ...
   *
   * <p>When possible, use the APIs which take a {@link java.nio.charset.Charset} and pass in {@link
   * com.google.common.base.Charsets#UTF_8} instead of using the String encoding method.
   */
  ...
  /**
   * Default native library folder name inside the project, i.e. "libs" While the folder inside the
   * .apk is "lib", we call that one libs because that's what we use in ant for both .jar and .so
   * and we need to make the 2 development ways compatible.
   */
   ...
   *
   * <p>Note this is not the same as {@link #FD_SOURCES} which is the folder name where sources are
   ...
   * containing one or more sub-folders (one per sample). However some older extras might contain a
   * single "sample" folder with directly the samples files in it. When possible we should encourage
  ...
  /**
   * Path of the documentation directory relative to the sdk folder. This is an OS path, ending with
   * a separator.
   */
  public static final String OS_SDK_DOCS_FOLDER = FD_DOCS + File.separator;
  /**
   * Path of the tools directory relative to the sdk folder, or to a platform folder. This is an OS
  ...
  /**
   * Path of the Platform tools Lib directory relative to the sdk folder. This is an OS path, ending
   * with a separator.
   */
  public static final String OS_SDK_PLATFORM_TOOLS_LIB_FOLDER =
      OS_SDK_PLATFORM_TOOLS_FOLDER + FD_LIB + File.separator;
  /**
   * Path of the build tools directory relative to the sdk folder. This is an OS path, ending with a
   * separator.
   */
  ...
  /**
   * Path of the images directory relative to a platform or addon folder. This is an OS path, ending
   * with a separator.
   */
  ...
  /**
   * Path of the resources directory relative to a platform folder. This is an OS path, ending with
   * a separator.
   */
  ...
  /**
   * Path of the samples directory relative to a platform folder. This is an OS path, ending with a
   * separator.
   */
  public static final String OS_PLATFORM_SAMPLES_FOLDER = FD_SAMPLES + File.separator;
  /**
   * Path of the android source directory relative to a platform folder. This is an OS path, ending
   * with a separator.
   */
  ...
  /**
   * Path of the Ant build rules directory relative to a platform folder. This is an OS path, ending
   * with a separator.
   */
  ...
  /**
   * MockView is part of the layoutlib bridge and used to display classes that have no rendering in
  ...
  public static final String OS_PLATFORM_ATTRS_XML =
      OS_PLATFORM_RESOURCES_FOLDER + SdkConstants.FD_RES_VALUES + File.separator + FN_ATTRS_XML;
  ...
  /**
   * Returns the appropriate name for the 'android' command, which is 'android.exe' for Windows and
   ...
   * and 'mkdsdcard' for all other platforms.
   */
   ...
   *
   * @return one of {@link #PLATFORM_WINDOWS}, {@link #PLATFORM_DARWIN}, {@link #PLATFORM_LINUX} or
