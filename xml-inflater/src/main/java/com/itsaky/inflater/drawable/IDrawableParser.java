...
public abstract class IDrawableParser extends CommonParseUtils {
    ...
    /**
     * Checks if the drawable parser is allowed to parse at the current depth. This must be checked
     * if the drawable parser keeps looking for {@link XmlPullParser#START_TAG} or {@link
     * XmlPullParser#END_TAG}.
     *
     * <p>The {@link LayerListParser} can contain tags which are expected to be parsed by other
     * parsers. So, if the nesting parser does not check if this method if <code>true</code>, it
     * might consume all the events and the {@link LayerListParser} will have invalid data to parse.
     * In some cases, the nested parser might consume all the events until {@link
