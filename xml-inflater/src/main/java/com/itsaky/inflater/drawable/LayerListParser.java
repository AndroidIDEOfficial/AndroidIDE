...
public class LayerListParser extends IDrawableParser {
    ...
    protected LayerListParser (XmlPullParser parser, IResourceTable resourceFinder, DisplayMetrics displayMetrics, int minDepth) {
            ...
            if (event == XmlPullParser.START_TAG) {
                ...
                if ("item".equals (name)) {
                    ...
                    if (index != -1) {
                        ...
                        // if the android:drawable is specified, we do not parse the inner elements of <item>
                        parseInner = false;
                    }
                    ...
                    final var parser = DrawableParserFactory.parserForTag (null, resourceFinder, displayMetrics, this.parser, name);
                    if (parser != null) {
    ...
    private void skipToEndTag (int depth) throws Exception {
        for (var event = parser.getEventType (); event != XmlPullParser.END_DOCUMENT; event = parser.next ()) {
