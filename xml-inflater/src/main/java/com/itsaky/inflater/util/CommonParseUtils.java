...
public class CommonParseUtils {
        ...
        if (Character.isDigit(c)) {
            ...
            return (int)
                    TypedValue.applyDimension(
                            getUnitForDimensionType(dimensionType), dimen, displayMetrics);
        } else if (c == '@') {
    ...
    protected int parseColor(String color, final Context ctx) {
        if (HEX_COLOR.matcher(color).matches()) {
            ...
            final int id =
                    findFrameworkResourceId("color", color.substring("@android:color/".length()));
        ...
        if (HEX_COLOR.matcher(value).matches()) {
            ...
            if (value.startsWith("@android:")) {
                ...
                if (value.startsWith("@drawable/")) {
                    final File drawable =
                            resourceFinder.findDrawable(value.substring("@drawable/".length()));
                    ...
                    try {
                        final var parser =
                                DrawableParserFactory.newParser(ctx, drawable, resourceFinder);
                        if (parser == null) {
                            return null;
                        }
                        return parser.parse();
                    ...
                    final String color =
                            resourceFinder.findColor(value.substring("@color/".length()));
