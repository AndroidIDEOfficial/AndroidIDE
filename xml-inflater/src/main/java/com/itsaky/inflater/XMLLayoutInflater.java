...
class XMLLayoutInflater extends BaseLayoutInflater {
    ...
    XMLLayoutInflater(@NonNull LayoutInflaterConfiguration config) {
        ...
        Preconditions.assertAllNotNull(
                "LayoutInflater parameters cannot be null",
                resDirs,
                attrInfo,
                widgetInfo,
                resFinder);
        ...
        try {
            ...
            LOG.error(
                    BaseApplication.getBaseInstance()
                            .getString(
                                    com.itsaky.inflater.R.string.err_no_attr_adapter,
    ...
    protected IView createFromInclude(Attributes attrs, IViewGroup parent) {
        ...
        Preconditions.assertNotBlank(
                layout,
                "'layout' attribute for <include> tag has invalid value."
                        + "layout='"
                        + layout
                        + "'");

        if (layout.startsWith("@android:layout/")) {
            return inflateAndroidLayout(
                    layout.substring("@android:layout/".length()), attrs, parent);
        }
    ...
    protected IView inflateAndroidLayout(
            String name, Attributes attrs, @NonNull IViewGroup parent) {
    ...
    protected IView createFromSimpleName(String name, ViewGroup parent, int style)
        ...
        if (widget == null) {
            ...
            return onCreateErrorView(
                    name,
                    getString(com.itsaky.inflater.R.string.msg_cannot_create_view, name),
                    parent);
        }
    ...
    protected IView createFromQualifiedName(String name, ViewGroup parent, int style)
        ...
        try {
            ...
            if (th instanceof NotSupportedException) {
                return onCreateErrorView(
                        name, getString(R.string.msg_view_not_supported, name), parent);
            }
    ...
    protected IView onCreateErrorView(String name, String msg, final ViewGroup parent)
        ...
        final var view =
                ErrorUiView.create(
                        Objects.requireNonNull(contextProvider.getContext(), "Context is null."),
                        name,
                        msg);
        return applyLayoutParams(view, parent);
    }
