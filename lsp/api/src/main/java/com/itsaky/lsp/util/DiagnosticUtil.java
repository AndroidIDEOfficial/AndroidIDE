...
public class DiagnosticUtil {
    ...
    private static int findLeftBound(List<DiagnosticItem> diagnostics, @NonNull Position position) {
        final var index =
                binarySearchDiagnosticPosition(
                        diagnostics, position.getLine(), position.getColumn());
        if (index == -1) {
            return -1;
        }
     ...
     * @return The index of the diagnostic item.
     */
    private static int findRightBound(
            List<DiagnosticItem> diagnostics, @NonNull Position position) {
        final var index =
                binarySearchDiagnosticPosition(
                        diagnostics, position.getLine(), position.getColumn());
        if (index == -1) {
