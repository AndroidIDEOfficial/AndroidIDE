...
public class TranslatePositionsVisitor extends ErrorAwareTreeScanner<Void, Void> {
  ...
  public TranslatePositionsVisitor(
  ...
  public Void scan(Tree node, Void p) {
    ...
    if (active && node != null) {
      Integer pos = endPos.replaceTree((JCTree) node, null);
      if (pos != null) {
        int newPos;
        if (pos < 0) {
          newPos = pos;
