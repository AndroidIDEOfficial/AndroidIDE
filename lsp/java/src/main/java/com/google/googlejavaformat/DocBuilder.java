...
public final class DocBuilder {
     ...
     * sometimes emit lines that are too long, or cheat by using a narrower line length to avoid
     * such problems.
     *
     * <p>One solution to this problem is not to decide whether a {@link Doc.Level} should be broken
     * until later (in this case, after the semicolon has been seen). A simpler approach is to
     * rewrite the {@link Doc} as here, so that the semicolon moves inside the inner {@link Doc},
     * and we can decide whether to break that {@link Doc} without seeing later text.
     */
