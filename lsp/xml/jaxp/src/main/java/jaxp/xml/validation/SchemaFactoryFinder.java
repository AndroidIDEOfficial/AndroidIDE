...
/**
 * Implementation of {@link jaxp.xml.validation.SchemaFactory#newInstance(String)}.
 *
...
class SchemaFactoryFinder  {
     ...
     *
     * @param loader
     *      to be used to load resource, {@link jaxp.xml.validation.SchemaFactory}, and
    ...
    /**
     * <p>Creates a new {@link jaxp.xml.validation.SchemaFactory} object for the specified
     * schema language.</p>
     *
     * @param schemaLanguage
     *      See {@link jaxp.xml.validation.SchemaFactory Schema Language} table in <code>SchemaFactory</code>
     ...
     */
    public jaxp.xml.validation.SchemaFactory newFactory(String schemaLanguage) {
        if(schemaLanguage==null) {
            throw new NullPointerException();
        }
        jaxp.xml.validation.SchemaFactory f = _newFactory(schemaLanguage);
        if (f != null) {
     ...
     */
    private jaxp.xml.validation.SchemaFactory _newFactory(String schemaLanguage) {
        jaxp.xml.validation.SchemaFactory sf;
        ...
        final jaxp.xml.validation.SchemaFactory factoryImpl = findServiceProvider(schemaLanguage);
        ...
        if(schemaLanguage.equals("http://www.w3.org/2001/XMLSchema")) {
            ...
            return createInstance("com.itsaky.org.apache.xerces.internal.jaxp.validation.XMLSchemaFactory", true);
        }
     ...
     */
    jaxp.xml.validation.SchemaFactory createInstance(String className ) {
        return createInstance( className, false );
    }

    jaxp.xml.validation.SchemaFactory createInstance(String className, boolean useServicesMechanism ) {
        jaxp.xml.validation.SchemaFactory schemaFactory = null;
        ...
        try {
                if (!jaxp.xml.validation.SchemaFactory.class.isAssignableFrom(clazz)) {
                    throw new ClassCastException(clazz.getName()
                                + " cannot be cast to " + jaxp.xml.validation.SchemaFactory.class);
                }
                if (!useServicesMechanism) {
                    schemaFactory = newInstanceNoServiceLoader(clazz);
                }
                if (schemaFactory == null) {
                    schemaFactory = (jaxp.xml.validation.SchemaFactory) clazz.newInstance();
                }
     ...
     *   method if available.
     */
    private static jaxp.xml.validation.SchemaFactory newInstanceNoServiceLoader(
         Class<?> providerClass
    ) {
    ...
    private boolean isSchemaLanguageSupportedBy(final jaxp.xml.validation.SchemaFactory factory,
     ...
     */
    private jaxp.xml.validation.SchemaFactory findServiceProvider(final String schemaLanguage) {
        assert schemaLanguage != null;
        // store current context.
        final AccessControlContext acc = AccessController.getContext();
        try {
            return AccessController.doPrivileged(new PrivilegedAction<jaxp.xml.validation.SchemaFactory>() {
                public jaxp.xml.validation.SchemaFactory run() {
                    final ServiceLoader<jaxp.xml.validation.SchemaFactory> loader =
                            ServiceLoader.load(SERVICE_CLASS);
                    for (jaxp.xml.validation.SchemaFactory factory : loader) {
    ...
    private static final Class<jaxp.xml.validation.SchemaFactory> SERVICE_CLASS = SchemaFactory.class;
