...
final class JavadocLexer {
     ...
     * to begin with.
     *
     * <p>Whitespace tokens are treated analogously. We don't really "want" to join whitespace
     * tokens, but in the course of joining literals, we incidentally join whitespace, too. We do
     * take advantage of the joining later on: It simplifies {@link #inferParagraphTags}.
     *
     * <p>Note that we do <i>not</i> merge a literal token and a whitespace token together.
     */
    private static ImmutableList<Token> joinAdjacentLiteralsAndAdjacentWhitespace(
            List<Token> input) {
        /*
        ...
        for (PeekingIterator<Token> tokens = peekingIterator(input.iterator());
                tokens.hasNext(); ) {
    ...
    /**
     * Replaces whitespace after a {@code href=...>} token with an "optional link break." This
     * allows us to output either {@code <a href=foo>foo</a>} or {@code <a href=foo>\nfoo</a>},
     * depending on how much space we have left on the line.
     *
     * <p>This method must be called after {@link #joinAdjacentLiteralsAndAdjacentWhitespace}, as it
     * assumes that adjacent whitespace tokens have already been joined.
     */
    private static ImmutableList<Token> optionalizeSpacesAfterLinks(List<Token> input) {
        ...
        for (PeekingIterator<Token> tokens = peekingIterator(input.iterator());
                tokens.hasNext(); ) {
            if (tokens.peek().getType() == LITERAL
                    && tokens.peek().getValue().matches("^href=[^>]*>")) {
                output.add(tokens.next());
