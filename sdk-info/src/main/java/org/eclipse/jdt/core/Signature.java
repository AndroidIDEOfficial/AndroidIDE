...
public final class Signature {
    ...
    public static final char C_UNRESOLVED =
            org.eclipse.jdt.internal.compiler.util.Util.C_UNRESOLVED;
    ...
    /**
     * Scans the given string for an capture type signature starting at the given index and appends
        ...
        if (start < string.length - 1) {
            ...
            if (string[start] != C_COLON) {
                throw new IllegalArgumentException(
                        "should be a colon at this location"); // $NON-NLS-1$
            }
    ...
    private static int appendTypeSignature(
        ...
        if (isVarArgs) {
            switch (c) {
                case C_ARRAY:
                    return appendArrayTypeSignature(
                            string, start, fullyQualifyTypeNames, buffer, true);
                case C_RESOLVED:
                case C_UNRESOLVED:
            ...
            switch (c) {
                ...
                case C_INTERSECTION:
                    return appendIntersectionTypeSignature(
                            string, start, fullyQualifyTypeNames, buffer);
                case C_STAR:
                case C_EXTENDS:
                case C_SUPER:
                    return appendTypeArgumentSignature(
                            string, start, fullyQualifyTypeNames, buffer);
                default:
                    throw newIllegalArgumentException(string, start);
            }
     ...
     * intersection type in source code notation using {@code &}. This method is equivalent to
     * <code>
     * createTypeSignature(new String(typeName),isResolved).toCharArray()</code>, although more
     * efficient for callers with character arrays rather than strings. If the type name is
     ...
     *     example, a type name from a binary class file), and <code>false</code> if the type name
     *     is to be considered unresolved (for example, a type name found in source code)
    ...
    public static char[] createTypeParameterSignature(
        ...
        if (length == 0) {
            return CharOperation.append(
                    typeParameterName,
                    C_COLON); // param signature with no bounds still gets trailing colon
        }
        int boundsSize = 0;
                ...
                default:
                    if (currentChar == ' ' || ScannerHelper.isWhitespace(currentChar)) {
                        if (lastAppendedChar == C_DOT) { // allow spaces after a dot
                            pos =
                                    consumeWhitespace(typeName, pos, length)
    ...
    private static int encodeTypeSignature(
        ...
        while (true) { // loop on type[&type]*
                ...
                if (checkPos > 0) {
                    ...
                    if ((pos = checkNextChar(typeName, '>', checkPos, length, true)) > 0) {
                        ...
                        while ((checkPos = checkNextChar(typeName, ',', pos, length, true)) > 0) {
                            pos =
                                    encodeTypeSignature(
                                            typeName, checkPos, isResolved, length, buffer);
                        }
            ...
            if (checkPos > 0) {
                if (buffer.charAt(0)
                        != C_UNION) // the constant name is wrong, its value is correct :-X
    ...
    public static char[] getSignatureSimpleName(char[] typeSignature) {
        ...
        if (dotCount > 0) {
            ...
            for (int i = 0; i < qualifiedType.length; i++) {
                ...
                if (dotCount <= 0) {
                    char[] simpleName =
                            CharOperation.subarray(qualifiedType, i + 1, qualifiedType.length);
        ...
        for (int idx = end; idx < length; idx++) {
            switch (parameterizedTypeSignature[idx]) {
                ...
                case C_GENERIC_END:
                    ...
                    if (deep < 0)
                        throw new IllegalArgumentException(
                                String.valueOf(parameterizedTypeSignature));
