...
public final class CharOperation {
     ...
     * case sensitive.
     *
     * <p>CamelCase denotes the convention of writing compound names without spaces, and
     * capitalizing every term. This function recognizes both upper and lower CamelCase, depending
     * whether the leading character is capitalized or not. The leading part of an upper CamelCase
     * pattern is assumed to contain a sequence of capitals which are appearing in the matching
     * name; e.g. 'NPE' will match 'NullPointerException', but not 'NewPerfData'. A lower CamelCase
     * pattern uses a lowercase first character. In Java, type names follow the upper CamelCase
     * convention, whereas method or field names follow the lower CamelCase convention.
     *
     * <p>The pattern may contain lowercase characters, which will be matched in a case sensitive
     * way. These characters must appear in sequence in the name. For instance, 'NPExcep' will match
     ...
     *
     * <ol>
     *   <li>pattern = "NPE".toCharArray() name = "NullPointerException".toCharArray() result =>
     *       true
     *   <li>pattern = "NPE".toCharArray() name = "NoPermissionException".toCharArray() result =>
     *       true
     ...
     * will match 'UTF16DocumentScannerSupport'.
     *
     * <p>CamelCase can be restricted to match only the same count of parts. When this restriction
     * is specified the given pattern and the given name must have <b>exactly</b> the same number of
     ...
     *   <li>pattern = "IPL3".toCharArray() name = "IPerspectiveListener3".toCharArray() result =>
     *       true
     *   <li>pattern = "HM".toCharArray() name = "HashMapEntry".toCharArray() result =>
     *       (samePartCount == false)
     * </ol>
     *
     * @param pattern the given pattern
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (nameChar < ScannerHelper.MAX_OBVIOUS) {
                    ...
                    if ((charNature & (ScannerHelper.C_LOWER_LETTER | ScannerHelper.C_SPECIAL))
                            != 0) {
    ...
    /**
     * Converts the given list of strings to an array of equal size, containing the individual
     * strings converted to char[] each.
     *
     * @param stringList
     * @return an array of char[], representing the elements in the input list, or {@code null} if
     *     the list was {@code null}.
     * @since 3.14
     */
     ...
     * zero length array. It answers null if the two arrays are null. If the first array is null,
     * then the second array is returned. If the second array is null, then the first array is
     * returned. if the second array is zero length array, the separator is appended. <br>
     * <br>
     * For example:
     *
     * <ol>
     ...
     * Answers the concatenation of the three arrays inserting the sep1 character between the first
     * two arrays and sep2 between the last two. It answers null if the three arrays are null. If
     * the first array is null or empty, then it answers the concatenation of second and third
     * inserting the sep2 character between them. If the second array is null or empty, then it
     * answers the concatenation of first and third inserting the sep1 character between them. If
     * the third array is null or empty, then it answers the concatenation of first and second
     * inserting the sep1 character between them. <br>
     * <br>
     * For example:
     *
     * <ol>
    ...
    /**
     * Answers the concatenation of the given array parts using the given separator between each
     * part irrespective of whether an element is a zero length array or not. <br>
     * <br>
     * For example:<br>
     *
     * <ol>
    ...
    public static final boolean equals(char[][] first, char[][] second, boolean isCaseSensitive) {
        ...
        for (int i = first.length; --i >= 0; )
            if (!equals(first[i], second[i], false)) return false;
        return true;
    }
    ...
    public static final boolean equals(
            char[] first, char[] second, int secondStart, int secondEnd) {
    ...
    public static final boolean equals(
        ...
        if (isCaseSensitive) {
            for (int i = first.length; --i >= 0; )
                if (first[i] != second[i + secondStart]) return false;
        } else {
            for (int i = first.length; --i >= 0; )
    ...
    public static final boolean fragmentEquals(
        ...
        if (ScannerHelper.toLowerCase(fragment[i])
                    != ScannerHelper.toLowerCase(name[i + startIndex])) return false;
        return true;
    }
    ...
    public static final int indexOf(
        ...
        if (toBeFoundLength == arrayLength) {
            if (isCaseSensitive) {
                ...
                for (int i = start; i < arrayLength; i++) {
                    if (ScannerHelper.toLowerCase(array[i])
                            != ScannerHelper.toLowerCase(toBeFound[i])) return -1;
                }
                return 0;
            }
        }
        if (isCaseSensitive) {
            ...
            for (int i = start, max = arrayLength - toBeFoundLength + 1; i < max; i++) {
                if (ScannerHelper.toLowerCase(array[i])
                        == ScannerHelper.toLowerCase(toBeFound[0])) {
                    for (int j = 1; j < toBeFoundLength; j++) {
                        if (ScannerHelper.toLowerCase(array[i + j])
                                != ScannerHelper.toLowerCase(toBeFound[j])) continue arrayLoop;
                    }
                    return i;
                }
            }
    ...
    public static final int lastIndexOf(
            char toBeFound, char[] array, int startIndex, int endIndex) {
    ...
    public static final boolean pathMatch(
        ...
        while (pSegmentStart < pLength
            ...
            fSegmentEnd =
                    CharOperation.indexOf(pathSeparator, filepath, fSegmentStart = fSegmentEnd + 1);
            // skip separator
            if (fSegmentEnd < 0) fSegmentEnd = fLength;
        }
        ...
        while (fSegmentStart < fLength) {
            ...
            if (pSegmentStart >= pLength) {
                ...
                pSegmentEnd =
                        CharOperation.indexOf(
                                pathSeparator, pattern, pSegmentStart = pSegmentRestart);
                if (pSegmentEnd < 0) pSegmentEnd = pLength;
                ...
                fSegmentEnd =
                        CharOperation.indexOf(
                                pathSeparator, filepath, fSegmentStart = fSegmentRestart);
                if (fSegmentEnd < 0) fSegmentEnd = fLength;
                continue checkSegment;
            }
            ...
            if (pSegmentEnd == pSegmentStart + 2
                ...
                pSegmentEnd =
                        CharOperation.indexOf(
                                pathSeparator, pattern, pSegmentStart = pSegmentEnd + 1);
                // skip separator
                if (pSegmentEnd < 0) pSegmentEnd = pLength;
                pSegmentRestart = pSegmentStart;
        ...
        if (!equals(toBeReplaced, replacementChars)) {
            ...
            for (int i = 0; i < max; ) {
                ...
                if (occurrenceCount == starts.length) {
                    System.arraycopy(
                            starts, 0, starts = new int[occurrenceCount * 2], 0, occurrenceCount);
                }
