...
public class TextViewAttrAdapter extends BaseViewAttrAdapter {
    ...
    public boolean apply(IAttribute attribute, View view, IResourceFinder resFinder) {
        if (super.apply(attribute, view, resFinder)) {
            return true; // Attribute was handled by BaseViewAttrAdapter
        }
    ...
    private int textStyleFor(String split) {
        switch (split) {
            case "bold" :
    ...
    private Typeface parseTypeface(String value) {
        switch (value) {
            case "sans" :
