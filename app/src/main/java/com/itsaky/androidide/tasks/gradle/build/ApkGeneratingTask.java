...
public abstract class ApkGeneratingTask extends BaseGradleTask {
    ...
    /**
     * Get the directories where APKs are generated.
     * These directories must contain an output-metadata.json file
     */
    ...
    public Set<File> getApks(File buildDir) {
        ...
        for (File dir : dirs) {
            ...
            try {
                ...
                element_finder: for (Element element : metadata.getElements()) {
                ...
                LOG.error(StudioApp.getInstance().getString(com.itsaky.androidide.R.string.err_cannot_parse_apk_metadata), th);
            }
        }
        return result;
    }
    ...
    private static final Logger LOG = Logger.instance("ApkGeneratingTask");
