...
public class CompletionListAdapter extends EditorCompletionAdapter {
    ...
    @Override
    public void attachValues (EditorAutoCompletion window, List<io.github.rosemoe.sora.lang.completion.CompletionItem> items) {
            ...
            binding.getRoot ()
                    .setBackgroundColor (
                            ContextCompat.getColor (
                                    getContext (), R.color.completionList_backgroundSelected));
        } else {
            binding.getRoot ()
                    .setBackgroundColor (
                            ContextCompat.getColor (
                                    getContext (), R.color.completionList_background));
        }
                ...
                () -> {
                    ...
                    if (hasRead && isValid) {
                        ...
                        if (kind == CompletionItemKind.METHOD
                                ...
                                && data.has ("memberName")) {
                            JsonElement erasedParameterTypesElement =
                                    data.get ("erasedParameterTypes");
                            if (erasedParameterTypesElement.isJsonArray ()) {
                                String simpleName = data.get ("memberName").getAsString ();
                                JsonArray erasedParameterTypes =
                                        erasedParameterTypesElement.getAsJsonArray ();
                                String[] paramTypes =
                                        new String[erasedParameterTypes.size ()];
                                for (int i = 0; i < erasedParameterTypes.size (); i++) {
                                    paramTypes[i] =
                                            erasedParameterTypes.get (i).getAsString ();
                                }
                        ...
                        if (info.deprecated > 0) {
                            infoBuilder.append (
                                    completionApiInfo
                                            .getContext ()
                                            .getString (
                                                    R.string.msg_api_info_deprecated,
                                                    info.deprecated));
                            infoBuilder.append ("\n");
                        }
                        ...
                        ThreadUtils.runOnUiThread (
                                () -> {
                                    if (infoBuilder.toString ().trim ().length () > 0) {
                                        completionApiInfo.setText (
                                                infoBuilder.toString ().trim ());
