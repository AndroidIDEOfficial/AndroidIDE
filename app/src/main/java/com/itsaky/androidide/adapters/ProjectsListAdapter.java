...
public class ProjectsListAdapter extends RecyclerView.Adapter<ProjectsListAdapter.VH> {
	...
	private final Comparator<AndroidProject> PROJECTS_SORTER = new Comparator<AndroidProject>(){
	...
	public ProjectsListAdapter(ArrayList<AndroidProject> mProjects, OnProjectClickListener listener) {
	...
	public ProjectsListAdapter.VH onCreateViewHolder(ViewGroup p1, int p2) {
		return new VH(LayoutProjectsItemBinding.inflate(LayoutInflater.from(p1.getContext()), p1, false));
	}
	...
	public void onBindViewHolder(ProjectsListAdapter.VH holder, int p2) {
		...
		holder.binding.deleteMsg.setText(holder.binding.deleteMsg.getContext().getString(R.string.msg_confirm_delete, project.getAppName()));
		if(!TextUtils.isEmpty(project.getIconPath())) Glide.with(holder.itemView.getContext().getApplicationContext()).load(new File(project.getIconPath())).into(holder.binding.projecsitemIcon);
		else holder.binding.projecsitemIcon.setImageResource(R.drawable.ic_android);
		
		holder.binding.projectsitemClickableContainer.setOnClickListener(v -> sendClickEvent(project));
		holder.binding.projectsitemClickableContainer.setOnLongClickListener(v -> holder.toggleDelete());
	...
	private void deleteProject(AndroidProject project, int index) {
		...
		new TaskExecutor().executeAsync(() -> FileUtils.delete(project.getProjectPath()), __ -> {
			StudioApp.getInstance().toast(__ ? R.string.msg_project_deleted : R.string.msg_project_delete_failed, Toaster.Type.SUCCESS);
		});
	}
	...
	public class VH extends RecyclerView.ViewHolder {
		...
		public void showConfirmDelete() {
			...
			TransitionManager.beginDelayedTransition(binding.getRoot(), new ChangeBounds().setDuration(85));
