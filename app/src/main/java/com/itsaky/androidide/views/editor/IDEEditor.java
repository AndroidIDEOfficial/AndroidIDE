...
public class IDEEditor extends CodeEditor {
      ...
      final CompletableFuture<SignatureHelp> future =
          CompletableFuture.supplyAsync(
              () -> {
                ...
                return provider.provideSignatures(
  ...
  public void didSave() {
    if (mLanguageServer != null && getFile() != null) {
      mLanguageServer.getDocumentHandler().onFileSaved(new DocumentSaveEvent(getFile().toPath()));
    }
  }
  ...
  public void close() {
    if (mLanguageServer != null && getFile() != null) {
      mLanguageServer.getDocumentHandler().onFileClosed(new DocumentCloseEvent(getFile().toPath()));
    ...
    return CompletableFuture.supplyAsync(
        () ->
            mLanguageServer
                ...
                .codeActions(
                    new CodeActionParams(getFile().toPath(), getCursorRange(), diagnostics)));
  }
  ...
  public void registerActionsTo(@NonNull ITextActionPresenter actionPresenter) {
    ...
    actionPresenter.registerAction(
        new TextAction(
            array.getDrawable(0), android.R.string.selectAll, TextAction.SELECT_ALL, index++));

    actionPresenter.registerAction(
        new TextAction(array.getDrawable(1), android.R.string.cut, TextAction.CUT, index++));

    actionPresenter.registerAction(
        new TextAction(array.getDrawable(2), android.R.string.copy, TextAction.COPY, index++));

    actionPresenter.registerAction(
        new TextAction(array.getDrawable(3), android.R.string.paste, TextAction.PASTE, index++));
  ...
  private void handleContentChange(ContentChangeEvent event) {
    ...
    StudioApp.getInstance()
        ...
        .getDocumentHandler()
        .onContentChange(new DocumentChangeEvent(getFile().toPath(), getText(), mFileVersion + 1));
  ...
  public interface ITextActionPresenter {
    ...
    /**
     * Destroy this action presenter. The presenter should unsubscribe from any subscribed events
