  ...
  private void createNewFile(final File f, boolean forceUnknownType) {
    if (forceUnknownType) {
      ...
      final boolean isJava =
          Pattern.compile(Pattern.quote(androidProject().getProjectPath()) + JAVA_PATH_REGEX)
              .matcher(f.getAbsolutePath())
              .find();
      final boolean isRes =
          Pattern.compile(Pattern.quote(androidProject().getProjectPath()) + RES_PATH_REGEX)
              .matcher(f.getAbsolutePath())
              .find();
      final boolean isLayoutRes =
          Pattern.compile(Pattern.quote(androidProject().getProjectPath()) + LAYOUT_RES_PATH_REGEX)
              .matcher(f.getAbsolutePath())
              .find();
      final boolean isMenuRes =
          Pattern.compile(Pattern.quote(androidProject().getProjectPath()) + MENU_RES_PATH_REGEX)
              .matcher(f.getAbsolutePath())
              .find();
      final boolean isDrawableRes =
          Pattern.compile(
                  Pattern.quote(androidProject().getProjectPath()) + DRAWABLE_RES_PATH_REGEX)
              .matcher(f.getAbsolutePath())
              .find();
      if (isJava) {
        createJavaClass(f);
