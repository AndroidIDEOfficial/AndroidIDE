...
public class IDEService {
            ...
            () -> {
              ...
              if (mIDEProject != null) {
                ...
                StudioApp.getInstance()
                    .toast(
                        com.itsaky.androidide.R.string.msg_init_project_failed, Toaster.Type.ERROR);
              }
            })
        .start();
  }
    ...
    final Runnable taskRunner =
        () -> {
            ...
            if (result.equals(Boolean.FALSE)) {
              if (listener != null) {
                ...
                listener.appendOutput(
                    task, getString(R.string.msg_gradlew_installation_failed, failure));
          ...
          if (listener != null) {
            ...
            currentTask = task;
            listener.appendOutput(
                task, getString(R.string.msg_task_begin, currentTime(), task.getName()));
            try {
  ...
  private void startReading(InputStream in) {
    final Thread reader =
        new Thread(new InputStreamLineReader(in, mOutputReadListener), "GradleProcessOutputReader");
  ...
  public void stopAllDaemons() {
    StudioApp.getInstance()
        ...
        .bgAppend(
            String.format(
                Locale.US, "cd '%s' && sh gradlew --stop", projectRoot.getAbsolutePath()));
    if (isBuilding()) {
      if (listener != null)
