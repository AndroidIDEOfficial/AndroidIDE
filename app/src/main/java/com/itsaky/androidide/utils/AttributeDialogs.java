...
/**
 * Helper class for creating dialogs for editing view attributes. <br>
 * TODO Should we merge this into {@link DialogUtils}? TODO An attribute can accept multiple types
 * of values (dimension, enum, reference, etc) Can we somehow combine the editors according to
 * attribute format?
 *
 * @author Akash Yadav
 */
public class AttributeDialogs {
    ...
    /**
     * Context that will be used for creating dialogs. This must be initialized by calling {@link
     * #init(Context)} and the reference must be released by calling {@link #release()}
     */
     ...
     *
     * @param listener The listener that will be invoked when the user clicks the positive button.
    ...
    public static AlertDialog stringEditor(OnClickListener listener) {
        ...
        builder.setPositiveButton(
                ...
                (dialog, which) -> {
                    final var newValue =
                            binding.valueField.getEditText().getText().toString().trim();
                    if (newValue.length() <= 0) {
                        StudioApp.getInstance()
                                .toast(
                                        mContext.getString(R.string.msg_invalid_attr_value),
                                        Toaster.Type.ERROR);
                        return;
                    }
                    if (listener != null) {
     ...
     *
     * @param value The current value of the attribute. Used for determining which item must be
     *     selected by default.
     * @param listener A listener that will be invoked when the user presses the positive button of
     *     the dialog. This listener is invoked only when the user passes a valid dimension value.
    ...
    public static AlertDialog dimensionEditor(CharSequence value, OnClickListener listener) {
        ...
        binding.choices.setOnCheckedChangeListener(
                (group, checkedId) ->
                        binding.otherValue.setVisibility(
                                binding.other.isChecked() ? View.VISIBLE : View.GONE));
     ...
     */
    public static AlertDialog enumEditor(
            CharSequence[] values, CharSequence value, OnClickListener listener) {
     ...
     * @param listener The click listener that will be invoked when an item is selected from the
     *     list.
     * @return The newly created dialog instance.
     */
    public static AlertDialog enumEditor(
            CharSequence[] values, int selected, OnClickListener listener) {
        final var builder = newMaterialDialogBuilder(mContext);
     ...
     */
    @NonNull
    public static AlertDialog flagEditor(
            @NonNull CharSequence[] values, @NonNull CharSequence value, OnClickListener listener) {
    ...
    @NonNull
    public static AlertDialog flagEditor(
            @NonNull CharSequence[] values, boolean[] selected, OnClickListener listener) {
        ...
        builder.setPositiveButton(
                ...
                (dialog, which) -> {
                    if (checked.size() <= 0) {
                        StudioApp.getInstance()
                                .toast(
                                        mContext.getString(R.string.msg_select_one_flag),
                                        Toaster.Type.ERROR);
                        return;
                    }
