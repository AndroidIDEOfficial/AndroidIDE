...
import static com.itsaky.lsp.models.HighlightTokenKind.*;
...
public class HighlightRangeHelper {
    ...
    /**
     * Check if the token at the given index has the given
     * token kind.
     *
     * @param line The line to check.
     * @param column The column to check.
     * @param kind The kind to check.
     * @return {@code true} if the token at the given position has the given kind, {@code false} otherwise.
     */
    public boolean isOfKind (int line, int column, HighlightTokenKind kind) {
        final var position = new Position (line, column);
        int left = 0; int right = this.highlights.size (); int mid;
        while (left < right) {
            mid = (left + right) / 2;
    ...
    private static final Logger LOG = Logger.instance ("HighlightRangeHelper");
}