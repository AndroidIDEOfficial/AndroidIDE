...
public class SdkHelper {
  ...
  public static String setupJDK() {
    ...
    join(sb);
    sb.append(
        "echo 'JAVA_HOME=/data/data/com.itsaky.androidide/files/usr/opt/openjdk' >>"
            + " $SYSROOT/etc/ide-environment.properties");
    join(sb);
    return sb.toString();
  }
  ...
  public static String setupZip(File f) {
    ...
    if (f.getName().endsWith(".tar.xz")) {
      ...
      if (f.getName().startsWith("cmdline-tools") || f.getName().startsWith("platform-tools")) {
        ...
        join(sb);
        sb.append("$BUSYBOX tar xvJf ")
            .append("$HOME/")
            .append(f.getName())
            .append(" -C $HOME/android-sdk");
      }
      // build tools content must be places in android-sdk/build-tools folder
        ...
        sb.append("$BUSYBOX tar xvJf ")
            .append("$HOME/")
            .append(f.getName())
            .append(" -C $HOME/android-sdk/build-tools");
      } else {
      ...
      // Regular Zips Found
      if (f.getName().startsWith("android-ndk-r24")) sb.append(setupNDK());
      else if (f.getName().startsWith("cmake")) sb.append(setupCMAKE());
      else {
  ...
  public static String setupNDK() {
    ...
    join(builder);
    builder
        .append("$BUSYBOX unzip $HOME/android-ndk-r24-aarch64.zip")
        .append(" -d $HOME/android-sdk/ndk/24.0.8215888");
    join(builder);
    builder.append(
        "ln -s $HOME/android-sdk/ndk/24.0.8215888/toolchains/llvm/prebuilt/linux-aarch64"
            + " $HOME/android-sdk/ndk/24.0.8215888/toolchains/llvm/prebuilt/linux-x86_64");
    join(builder);
    builder.append(
        "ln -s $HOME/android-sdk/ndk/24.0.8215888/prebuilt/linux-aarch64"
            + " $HOME/android-sdk/ndk/24.0.8215888/prebuilt/linux-x86_64");
    join(builder);
    // Cmake Config
    builder.append(
        "sed -i 's/if(CMAKE_HOST_SYSTEM_NAME STREQUAL Linux)/if(CMAKE_HOST_SYSTEM_NAME STREQUAL"
            + " Android)\n"
            + "set(ANDROID_HOST_TAG linux-aarch64)\n"
            + "elseif(CMAKE_HOST_SYSTEM_NAME STREQUAL Linux)/g'"
            + " $HOME/android-sdk/ndk/24.0.8215888/build/cmake/android-legacy.toolchain.cmake");
    join(builder);
    builder.append(
        "sed -i 's/if(CMAKE_HOST_SYSTEM_NAME STREQUAL Linux)/if(CMAKE_HOST_SYSTEM_NAME STREQUAL"
            + " Android)\n"
            + "set(ANDROID_HOST_TAG linux-aarch64)\n"
            + "elseif(CMAKE_HOST_SYSTEM_NAME STREQUAL Linux)/g'"
            + " $HOME/android-sdk/ndk/24.0.8215888/build/cmake/android.toolchain.cmake");
    join(builder);
