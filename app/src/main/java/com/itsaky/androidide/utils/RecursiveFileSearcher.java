...
public class RecursiveFileSearcher {
    ...
    public static void searchRecursiveAsync(
        ...
        new TaskExecutor()
                .executeAsync(
                        new Searcher(text, exts, searchDirs), result -> callback.onResult(result));
    }
                ...
                for (int j = 0; files != null && j < files.size(); j++) {
                    ...
                    while (matcher.find()) {
                        ...
                        final CharPosition start =
                                content.getIndexer().getCharPosition(matcher.start());
                        final CharPosition end =
                                content.getIndexer().getCharPosition(matcher.end());
                                ...
                                "..."
                                        .concat(
                                                text.substring(
                                                        ...
                                                        Math.min(
                                                                matcher.end() + 31, text.length())))
                                        .trim()
                                        .concat("...");
                        String match =
                                content.subContent(start.line, start.column, end.line, end.column)
                                        .toString();
                        ranges.add(
                                new SearchResult(range, file, sub.replaceAll("\\s+", " "), match));
                    }
                    if (ranges.size() > 0) {
