...
import com.itsaky.androidide.handlers.LspHandler.destroy
...
abstract class ProjectHandlerActivity : BaseEditorActivity(), IProjectHandler {
  ...
  private val mGradleServiceConnection: ServiceConnection =
    object : ServiceConnection {
      override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
        ...
        buildService.setEventListener(mBuildEventListener).startToolingServer {
          initializeProject()
        }
      }
  ...
  override fun initializeProject() {
    ...
    val future = buildService.initializeProject(projectDir.absolutePath)
    future.whenCompleteAsync { result: InitializeResult?, error: Throwable? ->
  ...
  override fun stopServices() {
    try {
      ...
      destroy(isChangingConfigurations)
    } catch (err: Throwable) {
      log.error("Unable to stop editor services. Please report this issue.", err)
    }
  }
  
  protected open fun onProjectInitialized() {
    setupProject()
    notifyProjectUpdate()
    ...
    builder.setPositiveButton(
      ...
      if (extensions.isNotEmpty()) {
        if (extensions.contains("|")) {
          for (str in extensions.split(Pattern.quote("|").toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()) {
            if (str.trim().isEmpty()) {
              continue
            }
      ...
      if (searchDirs.isEmpty()) {
        ...
        RecursiveFileSearcher.searchRecursiveAsync(
          text, extensionList, searchDirs) { results -> handleSearchResults(results) }
      }
    }
  ...
  open fun getProgressSheet(msg: Int): ProgressSheet? {
    ...
    mSearchingProgress = ProgressSheet().apply {
      isCancelable = false
