...
public class MainActivity extends StudioActivity implements View.OnClickListener, ProjectWriterCallback {
    ...
    private void createNewProject () {
        final String appName = Objects.requireNonNull (createLayoutBinding.createprojectTextAppName.getEditText ()).getText ().toString ().trim ();
        final String packageName = Objects.requireNonNull (createLayoutBinding.createprojectTextPackageName.getEditText ()).getText ().toString ().trim ();
    ...
    private boolean isValid (String appName, String packageName, int minSdk, int targetSdk) {
        return appName != null && appName.length () > 0
                ...
                && minSdk > 1 && minSdk < 99
                && targetSdk > 1 && targetSdk < 99;
    }
    
    private int getMinSdk () {
        try {
            return Integer.parseInt (Objects.requireNonNull (createLayoutBinding.createprojectTextMinSdk.getEditText ()).getText ().toString ());
        } catch (Exception e) {
    ...
    private int getTargetSdk () {
        try {
            return Integer.parseInt (Objects.requireNonNull (createLayoutBinding.createprojectTextTargetSdk.getEditText ()).getText ().toString ());
        } catch (Exception e) {
            getApp ().toast (e.getMessage (), Toaster.Type.ERROR);
        }
        return -1;
